...
@dataclass
class AgentStep:
    step_number: int; state: AgentState; input_data: Dict[str, Any] = field(default_factory=dict)
    output_data: Dict[str, Any] = field(default_factory=dict); duration_ms: float = 0; error: Optional[str] = None

@dataclass
class AgentResponse:
    message: str; steps: List[AgentStep] = field(default_factory=list); total_duration_ms: float = 0
    tools_used: List[str] = field(default_factory=list); token_usage: Dict[str, int] = field(default_factory=dict)

class AgentCore:
    def __init__(self, model_id: str = "zai.glm-4.7-flash", region: str = "us-east-1", system_prompt: str = "You are a helpful assistant.", max_iterations: int = 10, tools: Optional[List[BaseTool]] = None):
        self.model_id = model_id
        self.client = boto3.client(service_name="bedrock-runtime", region_name=region)
        self.system_prompt = system_prompt; self.max_iterations = max_iterations
    ...
    def register_tool(self, tool: BaseTool) -> None: self.tools[tool.name] = tool
    ...
    def run(self, user_message: str, session_id: Optional[str] = None) -> AgentResponse:
        start_time = time.time(); steps, tools_used, total_tokens = [], [], {"input": 0, "output": 0}
        ...
        for iteration in range(self.max_iterations):
            try:
                ...
                if stop_reason != "tool_use" or not tool_calls:
                    return AgentResponse(message=final_text, steps=steps, total_duration_ms=(time.time() - start_time) * 1000, tools_used=tools_used, token_usage=total_tokens)
                ...
                for tc in tool_calls:
                    ...
                    if self._on_tool_call: self._on_tool_call(tc["name"], tc["input"])
                ...
                self.memory.add_user_message(f"[Tool results: {len(tool_calls)} tools executed]")
                
            except Exception as e:
                logger.error(f"Agent error: {e}")
                return AgentResponse(message="I encountered an error.", steps=steps, total_duration_ms=(time.time() - start_time) * 1000, tools_used=tools_used, token_usage=total_tokens)
                
        return AgentResponse(message="Max iterations reached.", steps=steps, total_duration_ms=(time.time() - start_time) * 1000, tools_used=tools_used, token_usage=total_tokens)
